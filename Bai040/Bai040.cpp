...
int TongSoLeTrenCot(int[][500], int, int, int);
...
int main()
{
	...
	for (int i = 1; i <= 1; i++)
	{
		...
		int kq = TongSoLeTrenCot(a, m, n, c);

		cout << setw(8) << setprecision(5);
		cout << "Tong cac gia tri le tren cot " << c << " la: " << kq;
...
int TongSoLeTrenCot(int a[][500], int m, int n, int c)
{
	int s = 0;
	for (int i = 0; i < m; i++)
		if (a[i][c] % 2 != 0)
